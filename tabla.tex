...
\tikzset{
  ...
  note2/.pic = {
    \node at (0,0) {
      \begin{minipage}{\textwidth}
        \begin{tabla}{3}{c}
        \note{4.9}{$\bullet$ Tiles are stored left to right, up to down (23*42=966 B). \\ $\bullet$ Each object count is 2 bytes, thus there are 11 unused object counts (22 bytes). \\ $\bullet$ Objects are stored sorted by ID (see table). }\\
        \end{tabla}
      \end{minipage}
    };
  },
  note3/.pic = {
    \node at (0,0) {
      \begin{minipage}{\textwidth}
        \begin{tabla}{3}{c}
        \note{5.1}{Frame sample: \\ $\bullet$ Input: right + jump. \\ $\bullet$ Value: $2^1$ + $2^0$ = 3.}\\
        \end{tabla}
      \end{minipage}
    };
  },
  arrow/.pic = {
    \draw[right color = 3, left color = 3]
      (10.75:1.1) -- (6.5:1) arc (6.25:120:1) [rounded corners=0.5]
                  -- (120:0.9) [rounded corners=1]
                  -- (130:1.1) [rounded corners=0.5]
                  -- (120:1.3) [sharp corners]
                  -- (120:1.2) arc (120:5.25:1.2) [rounded corners=1]
                  -- (10.75:1.1)
                  -- (6.5:1)
                  -- cycle;
  }
...
\scalebox{1}{
  \begin{tikzpicture}
    % --------------------------------- FRAME ----------------------------------
    \draw[rounded corners = 0.25cm, line width = 0.5mm] (-15.4,5.25) rectangle (14,-31.75);

    \begin{scope}
      \draw[rounded corners = 0.25cm, line width = 0.5mm, fill=black!10!white] (-15,1.5) rectangle (-9.4,-1.5);
      \node[scale=1.25] at (-12.2,0.75) {{\color{1}\large{\textbf{Green: Query format}}}};
      \node[scale=1.25] at (-12.6,0) {{\color{2}\large{\textbf{Blue: Data format}}}};
      \node[scale=1.25] at (-13.4,-0.75) {{\color{3}\large{\textbf{Red: Notes}}}};
    \end{scope}

    % -------------------------------- HEADER ----------------------------------
    \begin{scope}[xshift=1.5cm,yshift=-2.5cm,scale=1.15]
      % --- BOX ---
      \draw[rounded corners = 0.25cm, line width = 0.5mm, fill=black!10!white] (-14.325,6.25) rectangle (0.45,3.75);

      % --- TEXT ---
      \node[scale=1.1,xshift=-1.9cm,yshift=-0.1cm] at (-2,5.5) {\Huge{\textbf{Query \& Data format}}};
      \node at (-0.9,4.35) {\large{\textbf{Author: Eddy}}};
      \node at (-1,4) {\large{\textbf{Date: Feb/2020}}};
    ...
    % --------------------------------- ARROWS ---------------------------------
    %\pic[color=black,rotate=80,scale=1.5] at (-11,-0.75) {arrow};
    %\pic[color=black,rotate=200,scale=1.5,xscale=-1] at (-6.2,-9.75) {arrow};

    % --------------------------------- TABLES ---------------------------------
    \pic at (10,0) {queries};
    \pic at (4,-5.4) {parameters};
    \pic at (21.5,-2.1) {categories};
    \pic at (30,-6) {objects};
    ...
    \begin{scope}[yshift=-2.25cm]
      \pic at (4,-15.4) {query_format};
      \pic at (4,-22.4) {note1};
    \end{scope}

    \begin{scope}[xshift=0.65cm,yshift=-2.25cm]
      \pic at (10.5,-14.65) {map_format};
      \pic at (10.5,-21.65) {note2};
    \end{scope}

    \begin{scope}[xshift=1.5cm,yshift=-2.25cm]
      \pic at (22.5,-16.15) {attract_format};
    \end{scope}

    \begin{scope}[xshift=1cm,yshift=-2.25cm]
      \pic at (16.5,-15.4) {replay_format};
      \pic at (16.5,-22.4) {note3};
    \end{scope}
    ...
    \node[anchor=north west] at (4.25,-31.25) {Eddy, 20-Feb-2020, https://github.com/edelkas/NPP\_sheet};
